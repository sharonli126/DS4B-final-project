...
### Longevity Diversity between four Order (Primatesm, Rodentia, Carivora and Artiodactyla)
```{r}
...
According to the histogram, the maximum longevity of Primates is spread mainly between 10 to 60 years, with few at above 60 or over 120 years. While longevity of Rodentia are all below 45 years, and half of them are below 10 years of lifespan, which is obviously lower than Primates in general. 


### Overall with "na"
```{r, fig.height=10}
ggplot(long_cleaned_data, aes(x = value, y = maximum_longevity_yrs)) +
  geom_point(alpha = .1) +
  geom_smooth(aes(color = order), method = "lm", alpha = .5, se = F) +
...
# Apply the function to each column of p-values in the summary dataframe
pvalue_summary$Overall <- sapply(pvalue_summary$Overall, convert_to_stars)
pvalue_summary$Primates <- sapply(pvalue_summary$Primates, convert_to_stars)
pvalue_summary$Rodentia <- sapply(pvalue_summary$Rodentia, convert_to_stars)
pvalue_summary$Carnivora <- sapply(pvalue_summary$Carnivora, convert_to_stars)
pvalue_summary$Artiodactyla <- sapply(pvalue_summary$Artiodactyla, convert_to_stars)
...
```{r}
long_na_rm_data <- cleaned_na_rm_data |>
pivot_longer(c(female_maturity_days, gestation_incubation_days, 
                 weaning_days, litter_clutch_size, 
                 litters_clutches_per_year, adult_weight_g), 
               names_to = "trait",
...
- move all the data cleaning, filtering to data cleaning rmd

- always mention why i use this and that test/ plot
