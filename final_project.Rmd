...
There are various of life-history traits in the class of mammalia, such as Primates tend to have longer lifespans, extended juvenile dependency periods, and fewer offspring, while most rodents reproduce quickly with shorter lifespans, having more offspring in their lifetime. Meanwhile, species of Carnivora and Artiodactyla, like lions and deer respectively, exhibit a range of lifespans and reproductive strategies, but their lifespan is relatively higher than Rodentia [2,3].

Therefore, to understand deeper of which trait correlate the most with lifespan in different order of mammalia, this study analyse a big database of aging and life-history traits in over 4600 animal species from AnAge, aiming to examine and compare life-history traits impact on maximum longevity among mammalia, particularly in four larger order group, including Primates, Rodentia, Carnivora and Artiodactyla.
...
Some weird observations were identified and noted in the excluded data, through grouping all value by genus, summeraising mean, range of max and min value were found. Suspected genus with a range close to the mean will be searched online. All data vary from information online will be noted in the excluded data.
...
The visualization above have shown the linear relationship of each traits with maximum longevity. All directions are the same in 
...
Comparing the result from LM and CT, they are not aligned, such as adult weight is highly significant but is having the least positive correlation overall, and both Primates and Rodentia are having highest degree of correlation with weaning day (milk feeding stop) but it is not significant in linear model. This result might due to the different, since the LM is using the data without missing value, which amount is less than a half of data with missing value. 
